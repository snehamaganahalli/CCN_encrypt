...
{
    BIGNUM *bnum = BN_new(), // BIGNUM from openssl library. It is the pointer to the group of 64 bits. BN_new() allocates the BIGNUM.
           *y = BN_new(),
           *c = BN_new(),
           *anum = BN_new(),
           *qpow_u = BN_new(),
    ...
    /* Copy first half of the string in out variable. */
    memcpy(out, in, inlen << 2);
    int u = floor2(inlen, 1);
    int v = inlen - u;
    /* Points to first and second half of the string. */
